...
data {
  ...
  int<lower = 0, upper = T> tasks_est_n;
  int<lower = 1, upper = T> tasks_est[tasks_est_n]; // aray of tasks to estimnate
...
//  b_tier ~ normal(-1, 5); // price elasticity of tiers
  ...
  // loglikelihood
    target += reduce_sum(AR_MNLwSimPop_max, tasks_est, splitsize,
