...
data {
  ...
  int<lower = 2> df; // df over P, recommend 5
  ...
  // array[4] real prior_cov_scale;  // lb, ub, mean, sigma
...
transformed data{
 ...
 // matrix[P,P] L = cholesky_decompose(prior_cov/df_tot); // For Wishart
  ...
  vector[P] beta_a = rep_vector(1.05, P);  // initial alpha for beta distribution
  vector[P] beta_b = rep_vector(1.05, P);  // initial beta for beta distribution (update for constrained)
  ...
  if (con_n > 0){
    ...
    for (i in 1:P){
      if (con_sign[i] > 0){
        ...
        beta_a[i] = 1; // lower bound will be mode
      } 
      if (con_sign[i] < 0){
        a_UB[i] = con_factors[2]; // Neg Con UB alpha before hinge
        beta_b[i] = 1; // upper bound will be mode
      } 
    }   
  }
...
model {
  ...
  target+= normal_lupdf(alpha|prior_alpha, a_sig);
