...
functions{
  ...
  real MNL_LL_par(array[] int array_slice,
                  ...
                  int att_thresh_pos, int att_thresh_neg, vector thresh_neg, vector thresh_pos,
                  vector swipepos_dev, vector swipeneg_dev
  ) {
    real ll = 0;
    for (t in a_beg:a_end){
      ...
      if (att_thresh_pos == 1){
          ll+= -sum(log1p_exp(-100 *
                       (util - thresh_pos[task_individual[t]]) .*
                        swipepos_dev[start[t]:end[t]]
                        )); // Love > Like 
      }
      if (att_thresh_neg == 1){
          ll+= -sum(log1p_exp(-100 *
                       (util - thresh_neg[task_individual[t]]) .*
                        swipeneg_dev[start[t]:end[t]]
                        )); // Love > Like 
      } 
      if (task_type[t] == 1){ // Attraction task has multiple choices
         vector[num_items] wt_tasks = (wts_bound_att[1] + (wts_bound_att[2] - wts_bound_att[1]) *
                 inv_logit(task_timez_flip[start[t]:end[t]] * timescale_att[task_individual[t]])) *
                 wts[t]; // weight for each row of attraction data
         vector[num_items] prob = inv_logit(util * beta_scale[task_individual[t]]); // applies scale factor to utilities   
        ...
        if (task_type[t] == 2){ // Conversion 
         real wt_task = (wts_bound_conv[1] + (wts_bound_conv[2] - wts_bound_conv[1]) *
                        inv_logit(task_timez_flip[start[t]] * timescale_con[task_individual[t]])) *
                        wts[t]; // only uses first time of task_timez_flip 
         ll+= wt_task * 
...
data {
  ...
  vector[N] task_timez; // standardized time converted to weights
  ...
  vector[N * att_thresh_pos] swipepos_dev; // -1 = Like, 1 = Love, 0 = not a good swipe
  vector[N * att_thresh_neg] swipeneg_dev;  // -1 = Hate, 1 = Dislike, 0 = not a bad swipe
...
transformed data{
  vector[N] task_timez_flip = -1 * task_timez; // faster times will be bigger numbers
  vector[I] scale_1 = rep_vector(1, I); // scale factor of 1
...
parameters {
  ...
  vector<lower = scale_bounds[1], upper = scale_bounds[2]>[I * est_scale_util] beta_scale;  // respondent specific scale factors
  ...
  vector<upper = 0>[I * att_thresh_neg] thresh_neg; // Resp specific threshold for Hate vs Dislike
  vector<lower = 0>[I * att_thresh_pos] thresh_pos; // Resp speccific threshold for Like vs Love  
  
  // resp scale: time --> wts for conversion, attraction.  Use unit scale for convenience
  // timescale ~ beta(1, timescale_hyper (data))
  vector<lower = 0, upper = 1>[I * est_conv] timescale_con_u; // conversion: timescal_con = 2* this
  vector<lower = 0, upper = 1>[I * est_att] timescale_att_u; // attraction: timescal_att = 2* this
}
...
model {
  ...
  target += reduce_sum(MNL_LL_par, array_slice, splitsize, 
                       ...
                       task_type, task_timez_flip,
                       (est_scale_util ? beta_scale : scale_1),  // utility scale conv --> attr
                       2 * timescale_con_u, 2 * timescale_att_u, // extend unit timescale to [0,2]
                       wts_bound_att, wts_bound_conv,
                       att_thresh_pos, att_thresh_neg, thresh_neg, thresh_pos, swipepos_dev, swipeneg_dev); 
} // End Model
