...
functions{
  real MNL_LL_resp_slice(array[] int resp_slice,
                  ...
                  matrix ind,  vector dep_b, vector dep_w, array[] real wts,
    ...
    for (resp in r_beg:r_end){
      ...
      vector[id_ranges[resp, 5]] util_all = ind[id_ranges[resp,3]:id_ranges[resp,4],] *
      ...
      for (t in id_ranges[resp, 1]:id_ranges[resp, 2]){
        ll+= (dot_product(
                dep_b[start[t]:end[t]],
                log_softmax(util_all[(start[t] - row_adjust):
                                       (end[t] - row_adjust)])
             ) +
             dot_product(
                dep_w[start[t]:end[t]],
...
data {
  ...
  // Main data
  vector<lower = 0, upper = 1>[N] dep_b; // Dep variable: best
  vector<lower = 0, upper = 1>[N] dep_w; // Dep variable: worst
...
model {
  ...
  target += reduce_sum(MNL_LL_resp_slice, resp_slice, splitsize,
                       ind, dep_b, dep_w, wts, start, end, task_individual, id_ranges,
