...
if (!is.null(indcode_spec)){
  if(add_none){
    ...
    if (sum(None$None) >0) indcode_spec$None <- usercode1(None, "None")
  }  
  ...
  data_stan <- prep_file_stan(idtaskdep = data_conjoint[,col_id_task_dep],
    ...
    if(add_none){
      ...
      cat("\nAdded variable None, chosen ",
             sum(None$None[data_conjoint[,col_id_task_dep[3]]> 0]),
             " times out of ", sum(None$None), " shows\n")
      } else cat("\nNone not Added -- No rows of all 0 found.\n")
    } 
...
```{r Aggregate Model}
if (est_aggmodel){
agg_beta <- est_agg_model(data_stan, maxit = 1000, reltol = 1e-5)
...
# ESTIMATE MODEL =============
#####  Specify Stan Model 
stan_file <- "BaseHB_wPairCon_v2.5_chol.stan" # Name of stan model in dir$stanmodel
out_prefix <- "Toothpaste_Chol"
...
if (min(HB_fit$return_codes() == 0)){
  ...
  if (data_stan$P_cov > 0){
    covariate_means <- get_covariates(HB_fit, data_stan)
