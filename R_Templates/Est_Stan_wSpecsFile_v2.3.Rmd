...
data_file <- "Data_Train.csv" # YOUR .csv or .rds with extension
specs_file <- "Coding_and_Constraints_Sawtooth.xlsx" # YOUR Coding and constraints file
out_prefix <- "F999" # YOUR name for output files (prefix)
...
#####  Run Stan Model 
HB_model <- cmdstan_model(file.path(dir_stanmodel,stan_file), quiet = TRUE,
...
HB_fit <- HB_model$sample(modifyList(data_stan, data_model),
                ...
                show_messages = FALSE,
                validate_csv = FALSE)
saveRDS(HB_fit, file.path(dir_work, "HB_fit.rds"))
...
if (max(HB_fit$return_codes() == 0)){
  checkconverge_export(stan_outname, data_stan$code_master, data_stan$resp_id,
                       nchains = HB_fit$num_chains(), dir_stanout, out_prefix, dir_work)
  eb_betas_est(data_stan, draws_beta, colMeans(utilities), r_cores,
               out_prefix, dir_work, cov_scale = 1, linux = TRUE)  
