...
transformed data{
  matrix[N, P] ind2; // ind_coded and ind_levels will expand to ind (below)
  ...
  for (i in 1:n_atts){
    ...
    if (code_blocks[i,5] > 0){ //already coded, just copy
      ...
      ind2[:,col_beg:col_end] = ind_coded[:,col_coded:(col_coded + col_end - col_beg)];
    } else {
      int col_beg = code_blocks[i,1];
      int col_end = code_blocks[i,2];
      ...
      for (n in 1:N){
        ...
        if ((lev >= 1) && (lev <= nrows)) ind2[n,col_beg:col_end] = code_att[lev,:];
        if (lev == 0) ind2[n,col_beg:col_end] = zeroes;
      } // end 1:N loop coding
      lev_col = lev_col + 1;
    } // end if  
  } // end for  
